...
public class HomeWork7 {
    ...
    public static List<String> methodOne(List<String> names) {
        List<String> finalList = names.stream().filter(c -> c.contains("King")).collect(Collectors.toList());
        return finalList;
    }
    ...
    public static List<String> methodTwo(List<String> names) {
        List<String> finalList = names.stream().map(o -> o.substring(o.indexOf(' ') + 1))
    ...
    public static List<String> methodThree(List<String> names) {
        List<String> topNames = names.stream().map(o -> o.substring(0, o.indexOf(' ')))
                .filter(s -> s.charAt(0) == 'L').distinct().collect(Collectors.toList());
        return topNames;
    }
    ...
    public static List<String> methodFour(List<String> names) {
        List<String> surnames = names.stream().map(o -> o.substring(o.indexOf(' ') + 1))
                .filter(s -> s.charAt(0) == 'W').distinct().sorted().toList();
        return surnames;
    }
    ...
    public static Integer methodFive(List<String> names) {
        int count = (int)names.stream().map(o -> o.substring(0, o.indexOf(' '))).distinct()
    ...
    public static Integer methodSix(List<String> names) {
        int countName = (int) names.stream().map(o -> o.substring(0, o.indexOf(' ')))
                .filter(s -> s.length() > 5).distinct().count();
        int countSurname = (int) names.stream().map(o -> o.substring(o.indexOf(' ') + 1))
                .filter(s -> s.length() > 5).distinct().count();

        return countSurname + countName - 2;   // поясните, пожалуйста, в чем ошибка, просто у меня получается 63 вместо 61
    }
    ...
    public static List<String> methodSeven(List<String> names) {
        List<String> result = names.stream().filter(o -> o.charAt(o.indexOf(' ') + 1) == 'K' ||
                o.charAt(o.indexOf(' ') + 1) == 'S').map(o -> o.substring(0, o.indexOf(' ') + 2) + ".")
                .distinct()
                .sorted()
